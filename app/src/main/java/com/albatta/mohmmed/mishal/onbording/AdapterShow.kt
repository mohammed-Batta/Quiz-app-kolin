...
class AdapterShow(val context:Context, private val list: ArrayList<PublicQuestion>): RecyclerView.Adapter<AdapterShow.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       ...
       holder.btn_edit.setOnClickListener {
            val intent=Intent(context,UpdateAct::class.java)
            intent.putExtra("id",l.id)
           context.startActivity(intent)
       }
        holder.btn_delete.setOnClickListener {
          val result=  db.deletePublicQuestion(l.id)
